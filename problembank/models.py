...
class BankAccount(models.Model):
    ...
    position = models.CharField(
        max_length=20, choices=Position.choices, default=Position.Member, verbose_name='جایگاه')
...
class Subtopic(models.Model):
    ...
    topic = models.ForeignKey(
        Topic, on_delete=models.CASCADE, verbose_name='موضوع', related_name='subtopics')
...
class Problem(models.Model):
    ...
    title = models.CharField(
        max_length=100, default='بدون عنوان', verbose_name='عنوان')

    topics = models.ManyToManyField(
        Topic, verbose_name='موضوع(ها)', blank=True, related_name='problems')
    subtopics = models.ManyToManyField(
        Subtopic, verbose_name='زیر موضوع(ها)', blank=True, related_name='problems')
...
class BaseSubmit(models.Model):
    ...
    problem = models.ForeignKey(
        Problem, on_delete=models.CASCADE, verbose_name='مسئله', related_name='%(class)s')
    respondents = models.ManyToManyField(
        BankAccount, verbose_name='پاسخ دهنده (ها)', related_name='%(class)s')
    received_at = models.DateTimeField(
        default=timezone.now, verbose_name='تاریخ دریافت مسئله')
    status = models.CharField(max_length=20, default=Status.Received,
                              choices=Status.choices, verbose_name='وضعیت تصحیح')
    delivered_at = models.DateTimeField(
        null=True, verbose_name='تاریخ دریافت پاسخ')
...
class JudgeableSubmit(BaseSubmit):
    ...
    judge_note = models.CharField(
        max_length=200, null=True, blank=True, verbose_name='نظر مصحح')
